 ...
 *  - I1: all ids validated before any exec (validatePackageName/BundleId/DeviceId)
 *  - I3: debuggable gate checked before opening adb forward
...
export class DebugController {
  ...
  constructor(
    private readonly androidAdb: AndroidAdbFactory = (deviceId) => makeAdbRunner(deviceId),
  ...
  async attach(opts: {
    ...
    if (opts.platform === "android") {
      // SECURITY I1: validate all agent-supplied strings before any exec.
      validatePackageName(opts.app);
      if (opts.deviceId) validateDeviceId(opts.deviceId);

      const adb = this.androidAdb(opts.deviceId);

      // SECURITY I3: debuggable gate — reject non-debuggable apps early.
      await assertAndroidDebuggable(adb, opts.app);
  ...
  async detach(sessionId: string): Promise<void> {
    ...
    // SECURITY I6: serialize teardown after any in-flight op on this session so
    // we don't destroy the socket/frames out from under a pending pauseState/step.
    await this.lock(sessionId, async () => {
      ...
      if (e.platform === "android") {
        ...
        await c.rpc("detach", { sessionId: e.iosSessionId });
      }
  ...
  removeBreakpoint(sessionId: string, breakpointId: string): Promise<void> {
    return this.lock(sessionId, async () => {
      ...
      if (e.platform === "android") {
        ...
        const c = await this.ios();
        await c.rpc("removeBreakpoint", { sessionId: e.iosSessionId, breakpointId });
      }
    });
  }
  ...
  poll(
    sessionId: string,
    cursor: number,
  ): Promise<{ events: unknown[]; nextCursor: number; alive?: boolean }> {
    return this.lock(sessionId, async () => {
      const e = this.get(sessionId);
  ...
  step(sessionId: string, threadId: string, action: "OVER" | "INTO" | "OUT"): Promise<void> {
    return this.lock(sessionId, async () => {
      ...
      if (e.platform === "android") {
        ...
        const c = await this.ios();
        await c.rpc("step", { sessionId: e.iosSessionId, threadId, action });
      }
    });
  }
  ...
  listSessions(): { sessionId: string; platform: DebugPlatform }[] {
    return [...this.sessions.entries()].map(([sessionId, e]) => ({
      sessionId,
      platform: e.platform,
    }));
  }
