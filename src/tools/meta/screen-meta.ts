...
const { meta, aliases } = createMetaTool({
  ...
  extraSchema: {
    ...
    maxWidth: { type: "number", description: "Max width in pixels (overrides preset; else preset or 540)" },
    maxHeight: { type: "number", description: "Max height in pixels (overrides preset; else preset or 960)" },
    quality: { type: "number", description: "JPEG quality 1-100 (overrides preset; else preset or 55)" },
