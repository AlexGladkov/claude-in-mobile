...
export async function generateDiffOverlay(
  ...
  // Different sizes = 100% change, return current as-is
  ...
  // Build ignore mask for fast lookup
  ...
  const grid = new Uint8Array(gridW * gridH); // 1 = has changed pixels

  // Clone current image for overlay
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (dr + dg + db > thresholdSum) {
        changedPixels++;
        // Apply orange overlay with alpha blending
  ...
  // Draw bounding boxes and region numbers on overlay
